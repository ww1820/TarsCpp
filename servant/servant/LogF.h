...
namespace tars
{
    ...
    public:
        virtual int onDispatch(tars::ReqMessagePtr _msg_)
        {
            ...
            pair<string*, string*> r = equal_range(__Log_all, __Log_all+2, (it==_msg_->response->status.end())?_msg_->request.sFuncName:it->second);
            ...
            switch(r.first - __Log_all)
            {
                case 0:
                {
                    ...
                    {
                        callback_logger_exception(_msg_->response->iRet);
                ...
                case 1:
                {
                    if (_msg_->response->iRet != tars::TARSSERVERSUCCESS)
                    {
                        callback_loggerbyInfo_exception(_msg_->response->iRet);
    ...
    public:
        virtual int onDispatch(tars::ReqMessagePtr _msg_)
        {
            ...
            pair<string*, string*> r = equal_range(__Log_all, __Log_all+2, string(_msg_->request.sFuncName));
            if(r.first == r.second) return tars::TARSSERVERNOFUNCERR;
            switch(r.first - __Log_all)
            {
                case 0:
    ...
    public:
        ...
        LogProxy* tars_hash(uint32_t key)
        {
            return (LogProxy*)ServantProxy::tars_hash(key);
        }

        LogProxy* tars_consistent_hash(uint32_t key)
        {
    ...
    public:
        virtual ~Log(){}
        virtual void logger(const std::string & app,const std::string & server,const std::string & file,const std::string & format,const vector<std::string> & buffer,tars::TarsCurrentPtr _current_) = 0;
        virtual void logger(std::string && app,std::string && server,std::string && file,std::string && format,vector<std::string> && buffer,tars::TarsCurrentPtr _current_) 
        { return logger(app, server, file, format, buffer, _current_); }
        static void async_response_logger(tars::TarsCurrentPtr _current_)
        {
        ...
        static void async_response_push_logger(tars::CurrentPtr _current_, const map<string, string> &_context = tars::Current::TARS_STATUS())
        {
            {
                tars::TarsOutputStream<tars::BufferWriterVector> _os;
                _current_->sendPushResponse( tars::TARSSERVERSUCCESS ,"logger", _os, _context);
            }
        }

        virtual void loggerbyInfo(const tars::LogInfo & info,const vector<std::string> & buffer,tars::TarsCurrentPtr _current_) = 0;
        virtual void loggerbyInfo(tars::LogInfo && info,vector<std::string> && buffer,tars::TarsCurrentPtr _current_) 
        { return loggerbyInfo(info, buffer, _current_); }
        static void async_response_loggerbyInfo(tars::TarsCurrentPtr _current_)
        {
            if (_current_->getRequestVersion() == TUPVERSION )
            {
                UniAttribute<tars::BufferWriterVector, tars::BufferReader>  _tarsAttr_;
        ...
        static void async_response_push_loggerbyInfo(tars::CurrentPtr _current_, const map<string, string> &_context = tars::Current::TARS_STATUS())
        {
            {
                tars::TarsOutputStream<tars::BufferWriterVector> _os;
                _current_->sendPushResponse( tars::TARSSERVERSUCCESS ,"loggerbyInfo", _os, _context);
            }
        }
            ...
            switch(r.first - __tars__Log_all)
            {
                case 0:
                {
                    ...
                    if (_current->getRequestVersion() == TUPVERSION)
                    {
                        ...
                        _tarsAttr_.get("format", format);
                        _tarsAttr_.get("buffer", buffer);
                    }
                    ...
                    logger(std::move(app),std::move(server),std::move(file),std::move(format),std::move(buffer), _current);
                ...
                case 1:
                {
                    ...
                    loggerbyInfo(std::move(info),std::move(buffer), _current);
                    if(_current->isResponse())
                    {
                        if (_current->getRequestVersion() == TUPVERSION)
                        {
